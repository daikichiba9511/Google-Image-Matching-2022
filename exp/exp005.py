...
add mask of Person as preprocess
...
def get_maskrcnn(num_classes: int = 2, pretrained: bool = False, hidden_layer: int = 256) -> MaskRCNN:
    """
    Args:
        num_classes: background and persion. so the number is 2

    Ref:
    * https://pytorch.org/tutorials/intermediate/torchvision_tutorial.html
    """
    model = torchvision.models.detection.maskrcnn_resnet50_fpn(pretrained=pretrained, pretrained_backbone=False)
    ...
    # def create_maskrcnn_detectron2() -> DefaultPredictor:
    #     cfg = get_cfg()
    ...
    #     # cfg.MODEL.WEIGHT = model_zoo.get_checkpoint_url("new_baselines/mask_rcnn_R_101_FPN_400ep_LSJ.py")
...
def apply_people_mask(image: torch.Tensor, masks: torch.Tensor) -> torch.Tensor:
    # image shape: (C, H, W)
    # mask shape: (mask_num, 1, H, W)
    assert len(image.shape) == 3
    assert len(masks.shape) == 4

    # check RGB image and dtype to use draw_segmentation_masks
    # assert image.shape[0] == 3 and image.dtype == torch.uint8

    # masks = masks.to("cpu")
    # masks = masks < 0.5
    # masks = masks.squeeze(1) < 0.5
    # image_to_draw = image.detach().clone()
    # for mask in masks:
    #     image_to_draw[:, mask] = black
    # masks = masks > 0.5
    # for mask in masks:
    #     image[mask] = 0.0
    for mask in masks:
    ...
    # for mask in masks:
    # image = image[i:, :, :, :] * (1 - masks[i, 0])
    # pdb.set_trace()
    # image = image * mask
    return image


def apply_people_bboxes(gray_image: torch.Tensor, bboxes: torch.Tensor) -> torch.Tensor:
    assert gray_image.shape[0] == 1
    assert bboxes.shape[1] == 4

    size = gray_image.shape[1] * gray_image.shape[2]
    mask = np.random.uniform(0.0, 1.0, size).reshape(gray_image.shape[1], gray_image.shape[2])
    mask = np.where(mask > 0.5, 1.0, 0.0)
    mask = torch.from_numpy(mask).unsqueeze(0).to(gray_image.device)

    assert gray_image.shape == mask.shape

    for bbox in bboxes:
        x1, y1, x2, y2 = map(int, bbox)
        gray_image[y1:y2, x1:x2] = gray_image[y1:y2, x1:x2] * mask[y1:y2, x1:x2]
    return gray_image


def plot_image_pair(image0: np.ndarray, image1: np.ndarray, save_fig_path: Path) -> None:
    assert (
        len(image0.shape) == 3 and len(image1.shape) == 3
    ), f"image0.shape => {image0.shape}, image1.shape => {image1.shape}"
...
def validate(
    matcher: KF.LoFTR,
    maskrcnn: MaskRCNN | DefaultPredictor,
    scaling_dict: dict[str, float],
    data_root: Path,
    ...
    for scene in scaling_dict.keys():
        ...
        for batch_index, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            # for detectron2
            # output0 = maskrcnn([{"image": image_0[0]}])
            # output1 = maskrcnn([{"image": image_1[0]}])
            ...
            if batch_index < 30:
                ...
                save_fig_path = Path(f"./output/image_pair/{image_name}.png")
                plot_image_pair(
                    image_0.detach().cpu().permute(0, 2, 3, 1).numpy()[0],
                    image_1.detach().cpu().permute(0, 2, 3, 1).numpy()[0],
                    save_fig_path,
                )
...
def main() -> None:
    ...
    # maskrcnn = create_maskrcnn_detectron2()
    maskrcnn = get_maskrcnn(pretrained=True, num_classes=2)
    maskrcnn = maskrcnn.to(device)
    maskrcnn.eval()

    # torch.save(maskrcnn.state_dict(), "./output/checkpoints/mask_rcnn_R_50_FPN_3x.ckpt")
    # return
